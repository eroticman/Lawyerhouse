...
<body class="animsition dashboard">
			                  		...
			                  		<div class="fixed-table-toolbar">
			                  			<div class="bs-bars pull-left">
			                  				<div class="btn-group hidden-sm-down" id="exampleToolbar" role="group">
							                    <button type="button" class="btn btn-success btn-outline btn-sm" onclick="window.location.href = 'team_add.php';">
			                  				...
			                  				<!-- <table id="exampleTablePagination" data-toggle="table" data-url="../assets/data/bootstrap_table_test.json" data-query-params="queryParams" data-mobile-responsive="true" data-height="400" data-pagination="true" data-icon-size="outline" data-search="true" class="table table-hover" style="margin-top: -36px;"> -->
			                    				...
			                    				<?php if(!empty($team_list)) : ?>
			                  					<tbody>
			                  						<?php 
													$i = 0;
													foreach($team_list as $team_detail) : 
													?>
			                  						<tr> 
			                  							<td class=""><?php echo $i+1; ?></td> 
			                  							<td style=""><?php echo $team_detail->name; ?></td><td style=><img src="../img/team/<?php echo $team_detail->id; ?>/<?php echo $team_detail->img_cover; ?>" style="width:200px;"></td>
			                  							<td style="">
														  <?php if(!empty($team_detail->updated)) {
																  echo date("d/m/Y", strtotime( $team_detail->updated ) ); 
															} else {
																echo '-';
															}
														  ?>
			                  							...
			                  							<td>
			                  								<button type="button" class="btn btn-round btn-warning btn-sm" onclick="window.location.href = 'team_edit.php?id=<?php echo $team_detail->id; ?>';"><i class="icon wb-pencil" aria-hidden="true"></i></button>
			                  								<button type="button" class="btn btn-round btn-danger btn-sm" onclick="delT(<?php echo $team_detail->id; ?>)"><i class="icon wb-close" aria-hidden="true"></i></button>
			                  							</td> 
			                  						</tr>
			                  						<?php
			                  							$i++;
		...
		<div class="modal-dialog modal-sm">
			<div class="modal-content">
				...
				<div class="modal-body">
					<h4>คุณต้องการลบรายชื่อใช่หรือไม่ ?</h4>
				</div>
				<div class="modal-footer">
					<input type="hidden" id="tid" name="tid">
					<button type="button" class="btn btn-success" onclick="deleteTeam()"><i class="fa fa-check"></i> Confirm</button>
     ...
     function deleteTeam()
    {
   		$.ajax({
		  	type 	: 'POST',
		  	url 	: 'queryDB.php',
		  	data 	: {tid:$('#tid').val(), action:'delTeam'},
