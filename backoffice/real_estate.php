...
<head>
	...
	<title>Real Estate | Lawyer House </title>
...
<body class="animsition dashboard">
	...
	<?php $real_estate_list = real_estate_list(); ?>
			                  		...
			                  		<div class="fixed-table-toolbar">
			                  			<div class="bs-bars pull-left">
			                  				<div class="btn-group hidden-sm-down" id="exampleToolbar" role="group">
							                    <button type="button" class="btn btn-success btn-outline btn-sm" onclick="window.location.href = 'real_estate_add.php';">
			                  		...
			                  		<div class="" style="padding-bottom: 0px;">
			                  			<div class="fixed-table-body">
			                  				...
			                  				<!-- <table id="exampleTablePagination" data-toggle="table" data-url="../assets/data/bootstrap_table_test.json" data-query-params="queryParams" data-mobile-responsive="true" data-height="400" data-pagination="true" data-icon-size="outline" data-search="true" class="table table-hover" style="margin-top: -36px;"> -->
			                    				...
			                    				<?php if(!empty($real_estate_list)) : ?>
			                  					<tbody>
			                  						<?php 
													$i = 0;
													foreach($real_estate_list as $real_estate_detail) : 
													?>
			                  						<tr> 
			                  							<td class=""><?php echo $i+1; ?></td> 
			                  							<td style=""><?php echo $real_estate_detail->real_estate_name; ?></td>
			                  							<td style="">
														  <?php if(!empty($real_estate_detail->updated)) {
																  echo date("d/m/Y", strtotime( $real_estate_detail->updated ) ); 
															} else {
																echo '-';
															}
														  ?>
			                  							...
			                  							<td>
			                  								<button type="button" class="btn btn-round btn-warning btn-sm" onclick="window.location.href = 'real_estate_edit.php?id=<?php echo $real_estate_detail->id; ?>';"><i class="icon wb-pencil" aria-hidden="true"></i></button>
			                  								<button type="button" class="btn btn-round btn-danger btn-sm" onclick="delR(<?php echo $real_estate_detail->id; ?>)"><i class="icon wb-close" aria-hidden="true"></i></button>
			                  							</td> 
			                  						</tr>
			                  						<?php
			                  							$i++;
			                  					...
			                  					<?php endif; /* if(!empty($real_estate_list)) : */ ?>
			                  				</table>
			                  			</div>
		...
		<div class="modal-dialog modal-sm">
			<div class="modal-content">
				...
				<div class="modal-body">
					<h4>คุณต้องการลบบริการใช่หรือไม่ ?</h4>
				</div>
				<div class="modal-footer">
					<input type="hidden" id="rid" name="rid">
					<button type="button" class="btn btn-success" onclick="deleteReal()"><i class="fa fa-check"></i> Confirm</button>
     ...
     function deleteReal()
    {
   		$.ajax({
		  	type 	: 'POST',
		  	url 	: 'queryDB.php',
		  	data 	: {rid:$('#rid').val(), action:'delRealEstate'},
